...
def library():
    ...
    with (
        patch(
            "integration_library_builtIn.PlatformLibrary.get_kubernetes_api_client"
        ) as mock_get_client,
        patch(
            "integration_library_builtIn.PlatformLibrary.KubernetesClient"
        ) as mock_k8s_cls,
...
def _make_container_sc(
    ...
    csc.capabilities = (
        capabilities if capabilities is not None else _make_capabilities()
    )
...
def _make_pod_sc(
    ...
    psc.seccomp_profile = (
        _make_seccomp() if seccomp_profile is _UNSET else seccomp_profile
    )
    return psc
...
def _make_container(
    ...
    c.image = image
    c.security_context = (
        _make_container_sc() if security_context is _UNSET else security_context
    )
...
class TestCheckPodHardeningRules:
    ...
    def test_ch1_fail_run_as_non_root_false(self):
        container = _make_container(
            security_context=_make_container_sc(run_as_non_root=False)
        )
        pod = _make_pod(
    ...
    def test_ch1_pass_when_excluded(self):
        ...
        pod = _make_pod(
            pod_sc=_make_pod_sc(run_as_non_root=False), containers=[container]
        )
    ...
    def test_ch2_fail_privileged_true(self):
        container = _make_container(
            security_context=_make_container_sc(
                privileged=True, allow_privilege_escalation=False
            )
        )
        pod = _make_pod(containers=[container])
        violations = self._check(pod)
    ...
    def test_ch5_fail_drop_does_not_contain_all(self):
        container = _make_container(
            security_context=_make_container_sc(
                capabilities=_make_capabilities(drop=["NET_ADMIN"])
            )
        )
        pod = _make_pod(containers=[container])
        violations = self._check(pod)
    ...
    def test_ch5_pass_drop_all_uppercase(self):
        container = _make_container(
            security_context=_make_container_sc(
                capabilities=_make_capabilities(drop=["ALL"])
            )
        )
        pod = _make_pod(containers=[container])
        assert not any("[CH5]" in v for v in self._check(pod))
    ...
    def test_ch6_pass_seccomp_runtime_default_on_pod(self):
        pod = _make_pod(
            pod_sc=_make_pod_sc(seccomp_profile=_make_seccomp("RuntimeDefault"))
        )
    ...
    def test_ch6_pass_seccomp_runtime_default_on_container(self):
        container = _make_container(
            security_context=_make_container_sc(
                seccomp_profile=_make_seccomp("RuntimeDefault")
            )
        )
        pod = _make_pod(containers=[container])
        assert not any("[CH6]" in v for v in self._check(pod))
    ...
    def test_ch6_fail_seccomp_unconfined(self):
        container = _make_container(
            security_context=_make_container_sc(
                seccomp_profile=_make_seccomp("Unconfined")
            )
        )
        pod = _make_pod(
    ...
    def test_ch6_pass_when_excluded(self):
        ...
        pod = _make_pod(
            pod_sc=_make_pod_sc(seccomp_profile=None), containers=[container]
        )
    ...
    def test_ch7_fail_bidirectional_mount(self):
        container = _make_container(
            volume_mounts=[
                _make_volume_mount(
                    name="data", mount_path="/data", mount_propagation="Bidirectional"
                )
            ]
        )
        pod = _make_pod(containers=[container])
    ...
    def test_ch11_fail_host_path_volume(self):
        pod = _make_pod(
            volumes=[_make_host_path_volume(name="host-data", path="/var/lib")]
        )
        violations = self._check(pod)
    ...
    def test_ch12_fail_secret_ref_in_env_from(self):
        ...
        violations = self._check(pod)
        assert any(
            "[CH12]" in v and "secretRef" in v and "app-secret" in v for v in violations
        )
...
class TestCheckContainerHardening:
    ...
    def test_multiple_violations_all_reported(self):
        ...
        c2 = _make_container(
            name="c2", security_context=_make_container_sc(privileged=True)
        )
    ...
    def test_all_exclusion_suppresses_rule_for_every_pod(self):
        ...
        lib = self._make_library_with_pods([p1, p2])
        lib.check_container_hardening(
            "kafka", namespace="ns", exclusions={"_all": "CH9"}
        )

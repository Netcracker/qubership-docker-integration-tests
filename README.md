...
* `ATP_REPORT_ENABLED` - Boolean string: `true` uploads Allure results and attachments to S3 (Helm `atpReport.enabled`), `false` disables uploads. Default: `false`
* `ATP_STORAGE_PROVIDER` - Storage provider type: `aws` (AWS S3), `minio` (MinIO), or `s3` (S3-compatible storage)
* `ATP_STORAGE_SERVER_URL` - S3 API endpoint URL (required for MinIO and S3-compatible storage, e.g., <https://minio.example.com> or `https://s3.us-east-1.amazonaws.com` for AWS)
...
**Note:** When `ATP_REPORT_ENABLED` is true, test results are uploaded to S3 with the following structure:
